...
jQuery(document).ready(function($){
    $('#as_upload_image').click(function(e) {
        e.preventDefault();
        var image = wp.media({
            title: 'Upload Image',
        ...
        .on('select', function(e){
            ...
            console.log(as_uploaded_image);
            var as_image_url = as_uploaded_image.toJSON().url;
            // Let's assign the url value to the input field
            $('#as_image_url').val(as_image_url);
        });
    });

    $('#as_bg_image').click(function(e) {
        e.preventDefault();
        var image = wp.media({
            title: 'Upload Image',
            // mutiple: true if you want to upload multiple files at once
        ...
        .on('select', function(e){
            // This will return the selected image from the Media Uploader, the result is an object
            var as_bg_url_image = image.state().get('selection').first();
            // We convert uploaded_image to a JSON object to make accessing it easier
            // Output to the console uploaded_image
            console.log(as_bg_url_image);
            var as_bg_url = as_bg_url_image.toJSON().url;
            // Let's assign the url value to the input field
            $('#as_bg_url').val(as_bg_url);
        });
    });


    $('.as_wp_color').wpColorPicker();
    $('.ranj_width').jRange({
        from: 320,
        to: 1500,
        step: 1,
